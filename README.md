...
#### 8. 把你的 IG 帳號訂閱到這個 App（容易漏掉的一步）

驗證 Webhook 網址成功，只代表 App 知道事件要送到哪裡；**還要另外把你的 IG 帳號「訂閱」到這個 App**，
Meta 才會真的開始送留言事件過來，不然 Webhook 永遠不會被觸發。

在瀏覽器打開（把 `你的網址` 和 `VERIFY_TOKEN` 換成你自己的值）：

```
https://你的網址/admin/subscribe?token=VERIFY_TOKEN
```

看到回應裡有 `"success": true` 就代表訂閱成功。這個網址平常不會用到，只有第一次設定、
或是重新產生 Access Token 之後需要再訂閱一次時才需要打開。
